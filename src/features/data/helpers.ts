...
export function calculateData() {
  ...
  if (bandExpenses > 0 && countWithBands > 0) {
    avgBandCost = fixed2Number((bandExpenses / countWithBands));
  }

  if (venueExpenses > 0 && countWithVenues) {
    avgVenueCost = fixed2Number((venueExpenses / countWithVenues));
  }

  if (count > 0 && numEvents > 0) {
    avgEvent = fixed2Number(Math.round(count / numEvents));
  }

  if (income > 0 && count > 0) {
    revper = fixed2Number((income / count)); // calculate the revenue per person for the year
  } else {
    revper = 0;
  }
...
export function getSplitExpensesPercent(expenses) {
  const splitExpenses = _.map(_.filter(expenses, (e) => {
    return e.type === 'Split';
  }), m => parseInt(m.percent, 10));


  const totalExpensesPercent = () => {
    if (splitExpenses.length > 1) {
      return _.reduce(splitExpenses, (a, b) => a + b);
    } else if (splitExpenses.length === 1) {
      return splitExpenses[0];
    }
    return 0;
  };

  return totalExpensesPercent();
}
...
export const getSplitExpenses = (expenses): { [expenseId: string]: IExpense } => {
  const splitExpenses = filterFunction(expenses, e => e.type === 'Split');

  return splitExpenses;
}

export const getFixedExpenses = (expenses): { [expenseId: string]: IExpense } => {
  const fixedExpenses = filterFunction(expenses, e => e.type === 'Fixed');

  return fixedExpenses;
}

export const findBandPayout = (expenses) => {
  const bandExpense = _.find(expenses, e => e.desc === 'Band');
  if (bandExpense) {
    return bandExpense.cost;
  }

  return 0;
}

export const fixed2Number = (number: number): number => {
